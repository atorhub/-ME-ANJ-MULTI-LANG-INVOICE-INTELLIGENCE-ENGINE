...
(function () {
  ...
  function parseRawInvoiceText(raw) {
    ...
    (function () {
      ...
      if (!parsed.merchant) {
        ...
        for (const l of lines) {
          if (l.length > best.length &&
            /[A-Za-z]/.test(l) &&
            !/invoice|bill|receipt/i.test(l)) best = l;
        }
    ...
    /* ---- date detect ---- */
    (function () {
      const cand = (raw.match(
        /(\d{1,2}[\/\-\.\s]\d{1,2}[\/\-\.\s]\d{2,4}|\d{4}[\/\-\.\s]\d{1,2}[\/\-\.\s]\d{1,2}|[A-Za-z]{3,9}\s+\d{1,2},\s*\d{4})/g
      ) || []);
      let dt = null;
    ...
    (function () {
      ...
      tail.forEach(l => {
        if (
          /total|grand total|net amount|amount due|balance due|payable|invoice total/i.test(l) ||
          /₹|\$|£|€|Rs\b|INR\b/i.test(l)
        ) {
          const nums = l.match(/-?[\d.,]+/g) || [];
          nums.forEach(n => {
            const p = parseNumberString(n);
            if (p !== null) cand.push(p);
          });
        }
      });
      ...
      if (cand.length)
        ...
        const all = (raw.match(/-?[\d,.]{2,}/g) || [])
          .map(n => parseNumberString(n))
          .filter(Boolean);
        if (all.length)
          parsed.total = {
            cents: Math.max(...all),
    ...
    (function () {
      ...
      for (let i = 0; i < merged.length; i++) {
        ...
        if (
          ...
          cur = (cur.replace(/-+$/, '') + ' ' + merged[i + 1]).trim();
          i++;
        }
        finalLines.push(cur);
      }
      ...
      function mapRowToItem(line) {
        const nums = (line.match(/-?[\d.,]+(?:\.\d{1,2})?/g) || [])
          .map(s => parseNumberString(s))
          .filter(n => n !== null);

        const currency = detectCurrency(line);
        let name = line
          .replace(/([₹$€£]?[-]?\d{1,3}(?:[0-9,]*)(?:\.\d{1,2})?)/g, ' ')
          .replace(/\s{2,}/g, ' ')
          .trim();
        ...
        if (nums.length >= 3) {
          ...
          for (let i = nums.length - 2; i >= 0; i--) {
            const cand = nums[i];
            if (Math.abs(cand) < Math.abs(item.total * 2)) {
              item.price = cand;
              break;
            }
          }
          ...
          if (b > a * 1.05) {
            ...
            if (a % 100 === 0) {
              ...
              item.price = b;
              item.total = Math.floor(item.price * item.qty);
            } else {
              item.price = a; item.total = b;
            }
          }
        ...
        if (item.price && !item.total)
          item.total = Math.floor(item.price * (item.qty || 1));

        if (item.total && !item.price && item.qty)
          item.price = Math.floor(item.total / (item.qty || 1));

        if (item.qty === null)
          item.qty = item.price && item.total
            ? Math.max(1, Math.round(item.total / item.price))
            : 1;

        item.price = item.price ? Number(item.price) : item.price;
        item.total = item.total ? Number(item.total) : item.total;

        return item;
      }

      const candidates = finalLines.filter(
        l => /[A-Za-z]/.test(l) && /[0-9]/.test(l)
      );
    ...
    if (!parsed.total && sumItems > 0) {
      parsed.total = {
        cents: sumItems,
        currency: detectCurrency(parsed.raw),
        inferred: true
      };
    }
    ...
    /* ---- mismatch ---- */
    if (parsed.total && sumItems > 0) {
      if (Math.abs(parsed.total.cents - sumItems) >
        Math.max(200, Math.round(parsed.total.cents * 0.05))) {
        parsed.mismatch = {
          total: parsed.total.cents,
          itemsSum: sumItems
        };
      }
    }
    ...
    if (!parsed.merchant || parsed.merchant === 'UNKNOWN')
      parsed.issues.push({ field: 'merchant', problem: 'missing' });

    if (!parsed.date)
      parsed.issues.push({ field: 'date', problem: 'missing' });

    if (!parsed.total)
      parsed.issues.push({ field: 'total', problem: 'missing' });

    if (!parsed.items || parsed.items.length === 0)
      parsed.issues.push({ field: 'items', problem: 'no_items' });
    ...
    parsed.display = {
      ...
      total: parsed.total
        ? formatCents(
            parsed.total.cents,
            parsed.total.currency === 'INR'
              ? '₹'
              : parsed.total.currency === 'USD'
              ? '$'
      ...
      items: (parsed.items || []).map(it => ({
        ...
        qty: it.qty || 1,
        price: it.price
          ? formatCents(it.price, it.currency === 'INR' ? '₹' : '$')
          : '-',
        total: it.total
          ? formatCents(it.total, it.currency === 'INR' ? '₹' : '$')
          : '-'
      }))
    };
  ...
  function renderInvoicePreview(parsed) {
    ...
    if ((parsed.issues || []).length === 0 && !parsed.mismatch) {
      ...
      (parsed.issues || []).forEach(i => {
        ...
        d.textContent = `Issue: ${i.field} — ${i.problem}`;
        issuesBox.appendChild(d);
      });
      if (parsed.mismatch) {
        const d = document.createElement('div');
        d.textContent =
          `Total mismatch: parsed ${parsed.mismatch.total} vs items sum ${parsed.mismatch.itemsSum}`;
        issuesBox.appendChild(d);
      }
    }
  }
  ...
  async function renderHistory() {
    ...
    all.forEach(inv => {
      ...
      row.textContent =
        `${inv.display?.merchant || inv.merchant || 'Invoice'} — ` +
        `${inv.display?.total || '-'}`;
  ...
  async function runQuickOCR(file) {
    ...
    const img = await fileToImageBlob(file);
    if (!img) return setStatus('Unsupported file', false);

    const text = await recognizeWithTesseract(img);
    lastOCR.quick = text;
    lastOCR.combined = text;
  ...
  async function runDualOCR(file) {
    ...
    const t1 = await recognizeWithTesseract(img);
    const t2 = await recognizeWithTesseract(img);
  ...
  function baseName() {
    return (parsedResult?.merchant || 'invoice') + '_' + Date.now();
  }
  ...
  function exportTXT() {
    if (!parsedResult) return setStatus('Nothing to export', false);
    downloadBlob(
      new Blob([parsedResult.raw || ''], { type: 'text/plain' }),
      baseName() + '.txt'
    );
  ...
  function exportCSV() {
    ...
    (parsedResult.display.items || []).forEach(i => {
  ...
  async function exportPDF() {
    if (!parsedResult) return setStatus('Nothing to export', false);
    if (!window.html2canvas || !window.jspdf) return setStatus('PDF libs missing', false);

    const canvas = await html2canvas(document.getElementById('previewContainer'), { scale: 2 });
  ...
  async function exportZIP() {
    if (!parsedResult) return setStatus('Nothing to export', false);
    if (!window.JSZip) return setStatus('JSZip missing', false);

    const zip = new JSZip();
    zip.file('invoice.json', JSON.stringify(parsedResult, null, 2));
    zip.file('raw.txt', parsedResult.raw || '');
  ...
  parseBtn?.addEventListener('click', () => {
    if (!lastOCR.combined) return setStatus('No OCR text available', false);
    parsedResult = parseRawInvoiceText(lastOCR.combined);
    renderInvoicePreview(parsedResult);
    saveInvoice(parsedResult);
  ...
  themeSelect?.addEventListener('change', () => {
    ...
    localStorage.setItem('anj_theme', themeSelect.value);
  });
  ...
  window.ANJ_DEBUG = {
    ocr: () => lastOCR,
    parsed: () => parsedResult,
    parseText: t => renderInvoicePreview(parseRawInvoiceText(t)),
    status: setStatus
  };
